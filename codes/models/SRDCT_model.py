...
class SRDCTModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        self.ortho_constraint = 0.5 * F.mse_loss(torch.matmul(self.dct_weight, self.dct_weight.T), eye, True)
        ...
        for i in range(DCT_weight.shape[0]):
            ...
            var_loss = self.cri_pix(torch.var(basis_item), torch.var(weight_item))
            self.complex_order_constraint = self.complex_order_constraint + var_loss

        l_pix = self.l_pix_w * self.cri_pix(self.fake_H, self.real_H) + 3.5*self.ortho_constraint + 0.75 * self.complex_order_constraint
...
class CustomerModel(BaseModel):
    def __init__(self, opt):
        super(CustomerModel, self).__init__(opt)
        ...
        # define network and load pretrained models
        self.netG = customer.define_model(opt).to(self.device)
        if opt['dist']:
            self.netG = DistributedDataParallel(self.netG, device_ids=[torch.cuda.current_device()])
        else:
            self.netG = DataParallel(self.netG)
